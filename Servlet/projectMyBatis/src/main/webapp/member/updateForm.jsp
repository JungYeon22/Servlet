...
<%
	...
	MemberDTO beforMemberDTO = memberDAO.memberById(id);
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
<form action="update.jsp" name="updateForm" method="post">
	<table border="1" cellspacing=0>
		<tr>
			<td align="center" width="70px">이름</td>
			<td>
				<input type="text" name="name" id="name" placeholder="이름 입력" value="<%=beforMemberDTO.getName() %>">
		...
		<tr>
			<th>아이디</th>
			<td>
				<input type="text" name="id" id="id" placeholder="아이디 입력" value="<%=id %>" readonly>
				<div id=checkId></div>
			</td>
		</tr>
		<tr>
			<th>비밀번호</th>
			<td>
				<input type="password" id="pwd" value="<%=beforMemberDTO.getPwd()%>"name="pwd">
				<div id=checkPwd></div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<th>이메일</th>
			<td>
				<input type="text" name="email1" id="email1" value="<%=beforMemberDTO.getEmail1()%>" size=10>@
				<input type="text" name="email2" id="email2" value="<%=beforMemberDTO.getEmail2()%>" size=10>
		...
		<tr>
			...
			<td>
				...
				-
				<input type="text" name="tel2" value="<%=beforMemberDTO.getTel2()%>" size=4 maxlength=4>
				-
				<input type="text" name="tel3" value="<%=beforMemberDTO.getTel3()%>" size=4 maxlength=4>
			</td>
		</tr>
		<tr>
			<th>주소</th>
			<td>
				<input type="text" name="zipcode" value="<%=beforMemberDTO.getZipcode()%>" id="zipcode" readonly>
				<button type="button" onclick="checkPost(); return false;">우편번호 검색</button><br>
				<input type="text" name="addr1" id="addr1" value="<%=beforMemberDTO.getAddr1()%>" size=50px readonly><br>
				<input type="text" name="addr2" id="addr2" value="<%=beforMemberDTO.getAddr2()%>" size=50px>
			</td>
		</tr>
		<tr>
...
window.onload=function(){
	document.getElementById("<%=beforMemberDTO.getGender()%>").checked = true;
	document.getElementById("tel1").value = "<%=beforMemberDTO.getTel1()%>" 
}
